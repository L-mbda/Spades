...
import { EndSession } from "@/app/components/endSession";
...
export default function Home() {
  ...
  const [sessionData, setSessionData] = useLocalstorageState<sessionInfo>(
    "sessionData",
    undefined,
  );
  const [opened, { open, close }] = useDisclosure(false);
  const [gameStore, setGameStore] = useLocalstorageState<gameData>(
    "gameStore",
    { gameInfo: [] },
  );
  ...
  function findWinner() {
    ...
    teamData.forEach((team) => {
      let points =
        gameStore.gameInfo.filter((data) => data.team == team).length == 0
          ? 0
          : gameStore.gameInfo
    ...
    return (
      <>
          ...
          <Modal.Body>
            ...
            >
              <TextInput
                label="Session Name"
                name="session_name"
                placeholder="Spades Tournament #6"
                required
              />
              <NumberInput
                label="Team Count"
              ...
              <Select
                ...
                label="Game Type"
                description="Gamemodes provide you most of the fun! Normal is where there is no loss with the formula (bet - gamesWon), but penalty is where you lose points with the set."
        ...
        <div className="flex flex-col p-10 items-center justify-center min-h-[100vh] dark:bg-[#1a1919] dark:text-white">
          ...
          <a
            onClick={open}
            className="mt-5 p-4 rounded-lg active:dark:bg-slate-700 cursor-pointer pl-16 pr-16 dark:text-white hover:dark:bg-slate-700 text-black dark:bg-slate-800 bg-gray-300 active:bg-gray-500 hover:bg-gray-400"
          >
            Start
          </a>
        </div>
  ...
  return (
    <>
      ...
      <div className="flex flex-col p-10 min-h-[100vh] dark:bg-[#1a1919] dark:text-white gap-4">
        ...
        <p>
          <i>Changes depending on if teams have any points in the record.</i>
        </p>
        <br />
        {/* Table */}
        <div className="overflow-x-auto w-full">
          <Table className="table-auto w-full" withColumnBorders>
            <Table.Thead className="w-[80%]">
              <Table.Tr>
                <Table.Td>Rounds</Table.Td>
                {sessionData.teamData.map((team, index) => (
                  <Table.Td key={index} abbr="Team Name">
            ...
            <Table.Tbody className="w-[80%]">
              {Array.from({ length: 13 }, (a, i) => (
                <Table.Tr key={i}>
                  <Table.Td>Round {i + 1}</Table.Td>
                  {sessionData.teamData.map((team, index) => (
                    <Table.Td
                      key={index}
                      className="min-w-[40vw] md:min-w-[10vw]"
                    >
                      <div className="flex flex-col gap-2">
                        <NumberInput
                          ...
                          onChange={(value) => {
                            ...
                            gameStoreData.forEach((masterData) => {
                              if (
                                masterData.roundNumber == i + 1 &&
                                masterData.team == team
                              ) {
                                gameData.push({
                                  team: team,
                                  // @ts-ignore
                          ...
                          placeholder="Bet"
                          label="Games bet on winning"
                          min={0}
                          defaultValue={
                            (
                              gameStore.gameInfo.find(
                                (data) =>
                                  data.roundNumber === i + 1 &&
                                  data.team === team,
                              ) || { gamesBet: 0 }
                        ...
                        <p>
                          Points Won:{" "}
                          {gameStore.gameInfo.filter(
                            (data) =>
                              data.roundNumber == i + 1 && data.team == team,
                          ).length == 0
                            ? 0
                            : gameStore.gameInfo
                                .filter(
              ...
              <Table.Tr>
                <Table.Td className="font-bold">Total Points</Table.Td>
                {/* Grabs total points per team */}
                {sessionData.teamData.map((team, index) => (
                  <Table.Td key={index}>
                    {gameStore.gameInfo.filter((data) => data.team == team)
                      .length == 0
                      ? 0
                      : gameStore.gameInfo
                          .filter((data) => data.team == team)
